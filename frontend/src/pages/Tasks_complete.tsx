...
const Tasks: React.FC = () => {
    ...
    Object.entries(tasksByClient).forEach(([clientId, clientTasks]) => {
      ...
      const tasksWithMonthlyTime: TaskWithMonthlyTime[] = clientTasks.map(task => {
        ...
        const taskTimersThisMonth = timers.filter(timer => {
          if (timer.taskId !== taskId) return false;
        ...
        return {
          ...
          totalTimeMinutes: task.timeSpent || 0,
  ...
  const handleReactivateTask = async (taskId: string) => {
    try {
      await tasksService.updateTask(taskId, { status: 'à faire' });
  ...
  const handleArchiveTask = async (taskId: string) => {
    try {
      await tasksService.updateTask(taskId, { isArchived: true });
      dispatch(addNotification({
                                        ...
                                        {(task.estimatedTime || 0) > 0 && (
                                          <div className="mt-3">
                                            ...
                                            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                              <div 
                                                className={`h-2 rounded-full transition-all ${
                                                  task.totalTimeMinutes > (task.estimatedTime || 0) 
                                                    ? 'bg-red-500' 
                                                    : 'bg-blue-500'
                                                }`}
                                                style={{ 
